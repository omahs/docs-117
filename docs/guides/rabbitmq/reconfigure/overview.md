...
This guide will give an overview on how KubeDB Ops-manager operator reconfigures `RabbitMQ` database components such as ReplicaSet, Shard, ConfigServer, Mongos, etc.
...
## How Reconfiguring RabbitMQ Process Works
...
<figure align="center">
  <img alt="Reconfiguring process of RabbitMQ" src="/docs/images/day-2-operation/RabbitMQ/mg-reconfigure.svg">
...
4. Then, in order to reconfigure the various components (ie. ReplicaSet, Shard, ConfigServer, Mongos, etc.) of the `RabbitMQ` database the user creates a `RabbitMQOpsRequest` CR with desired information.
