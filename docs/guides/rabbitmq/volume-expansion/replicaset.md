---
title: RabbitMQ Replicaset Volume Expansion
menu:
  docs_{{ .version }}:
...
# RabbitMQ Replicaset Volume Expansion

This guide will show you how to use `KubeDB` Ops-manager operator to expand the volume of a RabbitMQ Replicaset database.
...
Here, we are going to deploy a  `RabbitMQ` replicaset using a supported version by `KubeDB` operator. Then we are going to apply `RabbitMQOpsRequest` to expand its volume.
...
In this section, we are going to deploy a RabbitMQ Replicaset database with 1GB volume. Then, in the next section we will expand its volume to 2GB using `RabbitMQOpsRequest` CRD. Below is the YAML of the `RabbitMQ` CR that we are going to create,
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/RabbitMQ/volume-expansion/mg-replicaset.yaml
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/RabbitMQ/volume-expansion/mops-volume-exp-replicaset.yaml
RabbitMQopsrequest.ops.kubedb.com/mops-volume-exp-replicaset created
```

#### Verify RabbitMQ replicaset volume expanded successfully 
...
```bash
$ kubectl describe RabbitMQopsrequest -n demo mops-volume-exp-replicaset   
Name:         mops-volume-exp-replicaset
Namespace:    demo
Labels:       <none>
...
Metadata:
  ...
  Self Link:         /apis/ops.kubedb.com/v1alpha1/namespaces/demo/RabbitMQopsrequests/mops-volume-exp-replicaset
...
Status:
  Conditions:
    ...
    Type:                  VolumeExpansion
    Last Transition Time:  2020-08-25T18:22:38Z
    Message:               Successfully Resumed RabbitMQ: mg-replicaset
    Observed Generation:   1
...
Events:
  ...
  Normal  ResumeDatabase   3m11s  KubeDB Ops-manager operator  Resuming RabbitMQ
  Normal  ResumeDatabase   3m11s  KubeDB Ops-manager operator  Successfully Resumed RabbitMQ
...
The above output verifies that we have successfully expanded the volume of the RabbitMQ database.
...
kubectl delete RabbitMQopsrequest -n demo mops-volume-exp-replicaset
```