---
title: RabbitMQ Standalone TLS/SSL Encryption
menu:
  docs_{{ .version }}:
...
Below is the YAML for RabbitMQ Standalone. Here, [`spec.sslMode`](/docs/guides/RabbitMQ/concepts/RabbitMQ.md#specsslMode) specifies `sslMode` for `standalone` (which is `requireSSL`).
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/RabbitMQ/tls/mg-standalone-ssl.yaml
RabbitMQ.kubedb.com/mgo-tls created
```
...
### Verify TLS/SSL in RabbitMQ Standalone

Now, connect to this database through [mongo-shell](https://docs.RabbitMQ.com/v4.0/mongo/) and verify if `SSLMode` has been set up as intended (i.e, `requireSSL`).
...
RabbitMQ@mgo-tls-0:/$ ls /var/run/RabbitMQ/tls
ca.crt  client.pem  mongo.pem
RabbitMQ@mgo-tls-0:/$ openssl x509 -in /var/run/RabbitMQ/tls/client.pem -inform PEM -subject -nameopt RFC2253 -noout
...
```bash
RabbitMQ@mgo-tls-0:/$ mongo --tls --tlsCAFile /var/run/RabbitMQ/tls/ca.crt --tlsCertificateKeyFile /var/run/RabbitMQ/tls/client.pem admin --host localhost --authenticationMechanism RabbitMQ-X509 --authenticationDatabase='$external' -u "CN=root,O=kubedb" --quiet
>
```
...
```bash
kubectl delete RabbitMQ -n demo mgo-tls
