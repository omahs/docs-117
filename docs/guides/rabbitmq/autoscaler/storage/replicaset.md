...
# Storage Autoscaling of a RabbitMQ Replicaset Database

This guide will show you how to use `KubeDB` to autoscale the storage of a RabbitMQ Replicaset database.
...
Now, we are going to deploy a `RabbitMQ` replicaset using a supported version by `KubeDB` operator. Then we are going to apply `RabbitMQAutoscaler` to set up autoscaling.
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/RabbitMQ/autoscaling/storage/mg-rs.yaml
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/RabbitMQ/autoscaling/storage/mg-as-rs.yaml
...
Metadata:
  ...
  Self Link:         /apis/autoscaling.kubedb.com/v1alpha1/namespaces/demo/RabbitMQautoscalers/mg-as-rs
...
```bash
$ kubectl describe RabbitMQopsrequest -n demo mops-mg-rs-mft11m
...
Metadata:
  ...
  Self Link:               /apis/ops.kubedb.com/v1alpha1/namespaces/demo/RabbitMQopsrequests/mops-mg-rs-mft11m
...
Events:
  ...
  ----    ------                     ----   ----                        -------
  Normal  PauseDatabase              2m36s  KubeDB Ops-manager operator  Pausing RabbitMQ demo/mg-rs
  Normal  PauseDatabase              2m36s  KubeDB Ops-manager operator  Successfully paused RabbitMQ demo/mg-rs
  ...
  Normal  ResumeDatabase             81s    KubeDB Ops-manager operator  Resuming RabbitMQ demo/mg-rs
  Normal  ResumeDatabase             81s    KubeDB Ops-manager operator  Successfully resumed RabbitMQ demo/mg-rs
...
The above output verifies that we have successfully autoscaled the volume of the RabbitMQ replicaset database.
