...
# Storage Autoscaling of a RabbitMQ Sharded Database

This guide will show you how to use `KubeDB` to autoscale the storage of a RabbitMQ Sharded database.
...
Now, we are going to deploy a `RabbitMQ` sharded database using a supported version by `KubeDB` operator. Then we are going to apply `RabbitMQAutoscaler` to set up autoscaling.
...
```bash
$ kubectl create -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/RabbitMQ/autoscaling/storage/mg-sh.yaml
...
In order to set up vertical autoscaling for this sharded database, we have to create a `RabbitMQAutoscaler` CRO with our desired configuration. Below is the YAML of the `RabbitMQAutoscaler` object that we are going to create,
...
```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/RabbitMQ/autoscaling/storage/mg-as-sh.yaml
...
Metadata:
  ...
  Self Link:         /apis/autoscaling.kubedb.com/v1alpha1/namespaces/demo/RabbitMQautoscalers/mg-as-sh
...
```bash
$ kubectl describe RabbitMQopsrequest -n demo mops-mg-sh-ba5ikn
...
Metadata:
  ...
  Self Link:               /apis/ops.kubedb.com/v1alpha1/namespaces/demo/RabbitMQopsrequests/mops-mg-sh-ba5ikn
...
Events:
  ...
  ----    ------                ----   ----                        -------
  Normal  PauseDatabase         3m21s  KubeDB Ops-manager operator  Pausing RabbitMQ demo/mg-sh
  Normal  PauseDatabase         3m21s  KubeDB Ops-manager operator  Successfully paused RabbitMQ demo/mg-sh
  ...
  Normal  ResumeDatabase        36s    KubeDB Ops-manager operator  Resuming RabbitMQ demo/mg-sh
  Normal  ResumeDatabase        36s    KubeDB Ops-manager operator  Successfully resumed RabbitMQ demo/mg-sh
...
The above output verifies that we have successfully autoscaled the volume of the shard nodes of this RabbitMQ database.
